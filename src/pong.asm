...
;-- Ball animation ---
FrameCounter:      .res 1   ; counts frames
BallTileToggle:    .res 1   ; 0 or 1
...
.proc nmi_handler
  ...
  ; Increment frame counter
    ...
    ; Has 30 frames elapsed?
    CMP #30
    BCC SkipBallUpdate     ; If less than 30, skip

    ; Reset counter
    LDA #0
    STA FrameCounter

    ; Toggle BallTileToggle (0 -> 1, 1 -> 0)
    LDA BallTileToggle
    EOR #1                ; Flip bit 0
    STA BallTileToggle

    ; Update sprite tile
    ; Tile = 7 + BallTileToggle
    LDA BallTileToggle
    CLC
    ADC #6
    STA SPRITE_BALL_ADDR + 1    ; Write into tile index
SkipBallUpdate:
...
.byte 100, 8, $02, 20                   ; Top sprite: Y=100, Tile=8, Attr=0, X=20
.byte 108, 9, $02, 20                   ; Second sprite: Y=108, Tile=9, Attr=0, X=20
.byte 116, 9, $02, 20                   ; Third sprite: Y=116, Tile=9, Attr=0, X=20
.byte 124, 10, $02, 20                  ; Bottom sprite: Y=124, Tile=10, Attr=0, X=20
...
; Ball sprite
.byte 116, 6, 0, 120                  ; Ball: Y=116, Tile=6, Attr=0, X=120
